...
class UpdateProductController extends GetxController {
  ...
  initialiseData(EquipmentDetails details) async {
    nameController.text = details.name;
    modelController.text = details.model;
    serialNoController.text = details.serialNumber;
    detailsController.text = details.details;
    addressController.text = details.address;
    warrantyPeriodController.text = details.warrantyPeriodInYears;
    hasPeriodicMaintenance.value = details.isHavingPeriodicMaintenance;
    purchaseDateWrapper.value =
        getValidDateFormatStringFromTimestamp(details.purchaseDate) ?? '';
    if (details.isHavingPeriodicMaintenance) {
      scheduleMaintenancePeriodController.text =
          details.scheduleMaintenancePeriodInDays;
      lastPeriodicServiceDateWrapper.value =
          getValidDateFormatStringFromTimestamp(
    ...
    var userManagers = managerUsers
        .where((element) => element.code == details.userManagerCode)
        .toList();
    if (userManagers.isNotEmpty) {
      selectedUserManager.value = userManagers.first;
    }
    var userAdmins = adminUsers
        .where((element) => element.code == details.userAdminCode)
        .toList();
    if (userAdmins.isNotEmpty) {
      selectedUserAdmin.value = userAdmins.first;
    }
  }
  ...
  _fetchMasterData() async {
    ...
    this.managerUsers.value = adminUsers;
    this.adminUsers.value = adminUsers;
  }
  ...
  Future<void> saveAction(EquipmentDetails request) async {
    await Get.showOverlay<void>(
        asyncFunction: () async {
          if (requestFormKey.currentState!.validate()) {
            ...
            if (purchaseDateWrapper.value.isNotEmpty) {
            ...
            if (hasPeriodicMaintenance.value) {
              ...
              request.scheduleMaintenancePeriodInDays =
                  scheduleMaintenancePeriodController.text;
              if (lastPeriodicServiceDateWrapper.value.isNotEmpty) {
